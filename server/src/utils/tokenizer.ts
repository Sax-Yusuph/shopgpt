...
import { COMPLETION_MODEL, Message } from "./constants";

export const tokenizer = getEncoding("gpt2");
/**
 * Count the tokens for multi-message chat completion requests
 */
export function getChatRequestTokenCount(messages: Message[], model = COMPLETION_MODEL): number {
  const tokensPerRequest = 3; // every reply is primed with <|im_start|>assistant<|im_sep|>
  const numTokens = messages.reduce((acc, message) => acc + getMessageTokenCount(message), 0);
