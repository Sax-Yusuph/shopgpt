...
export async function openAiResponse(
  c: Context<{ Bindings: Bindings }, '/chat'>,
) {
  ...
  let prompts: Message[]
  const currentProductOrCollection = await getCurrentPageItems(
    pageType,
    tabUrl,
    storeUrl,
  )
  ...
  switch (pageType) {
    case PAGE_TYPE.PRODUCT:
      prompts = Prompts.product(
        contextText,
        storeName,
        currentProductOrCollection,
      )
      break
...
async function getCurrentPageItems(
  pageType: PAGE_TYPE,
  tabUrl: string,
  storeUrl: string,
) {
