  ...
  return (
    <div className="grid gap-2">
      <HoverCard openDelay={200}>
        ...
        <HoverCardContent align="start" className="w-[260px] font-mono text-sm" side="left">
          ...
          <HoverCard>
            <HoverCardContent side="left" align="start" forceMount className="min-h-[280px]">
              <div className="grid gap-2">
                ...
                <div className="font-mono text-sm text-muted-foreground">{peekedModel.description}</div>
                {peekedModel.strengths ? (
                  <div className="mt-4 grid gap-2">
                    <h5 className="text-sm font-medium leading-none">Strengths</h5>
                    <ul className="font-mono text-sm text-muted-foreground">{peekedModel.strengths}</ul>
                  </div>
                ) : null}
              </div>
            </HoverCardContent>
...
function ModelItem({ model, isSelected, onSelect, onPeek }: ModelItemProps) {
  ...
  return (
    <CommandItem
      ...
      onSelect={onSelect}
      ref={ref}
      className="font-mono aria-selected:bg-primary aria-selected:text-primary-foreground"
    >
      {model.name}
