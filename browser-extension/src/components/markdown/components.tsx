...
export const MarkdownComponents: Components = {
  ...
  img: ({ src, alt, className }) => (
    <div
      className={clx(
        'aspect-video bg-ui-bg-component rounded-md my-3 overflow-hidden',
        className,
      )}
    >
      <img
        src={src}
        alt={alt}
        className={'animate-in fade-in-75 w-full h-full object-cover'}
      />
    </div>
  ),
  ...
  kbd: ({ children, className }) => (
    <Kbd className={clx(className)}>{children}</Kbd>
  ),
  details: Details,
  ...
  ol: ({
    ...
    return (
      <ol
        {...props}
        className={clx('list-decimal px-1 my-3 space-y-5', className)}
      >
        {children}
      </ol>
    )
